...
vu16 ADC_Calibrator[4];      //遥控通道ADC校准值 volatile类型数据（表示不被优化的变量类型）
/*****************************************************************************
* 函  数：void ADC_DmaConfig(void)
* 功  能：初始化ADC采集通道
* 参  数：
* 返回值：no
* 备  注：
...
void ADC_DmaConfig(void)
{
	...
	ADC_Calibrator[1] = 1500;
	ADC_Calibrator[2] = 1000;
...
* 参  数：
* 返回值：no
* 备  注：对4路的AD采集值，进行一个求平均值
*****************************************************************************/
void ADC_Calibration(void)
{
	static u8 cnt=0; // 在中断调用，将变量初始化成static 类型的
