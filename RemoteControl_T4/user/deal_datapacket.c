...
//	printf("+++++++++++++++++++++++++++++++++++++++++++++++\r\n");
	...
	ADC_Value[0] = 1100 +1900 - ADC_ValueLimit(ADC_Value[0],1470,1530,1100,1900); // 摇杆方向与数值方向相反，需要减操作

	//ADC2 ---> Roll
	ADC_Value[1] = 1500 + ((1000+(ADC_ConvertedValue[1])*1000/4096) - ADC_Calibrator[1] );
	ADC_Value[1] = ADC_ValueLimit(ADC_Value[1],1470,1530,1100,1900);
	
	//ADC3 ---> Throttle
	ADC_Value[2] = 1000- (ADC_ConvertedValue[2]*1000/4096); //油门0~1000;
...
//=========直接采用指针操作内存中的数值将16位转成8位，速度快，并且不会发生精度截取的现象，还要注意，STM32是小端地址============
    ...
    packetData[2] = *(((u8*)ADC_Value)+6);        //ADC4 ---> Yaw
	packetData[3] = *(((u8*)ADC_Value)+7);
    packetData[4] = *(((u8*)ADC_Value)+4);        //ADC3 ---> Throttle 数据打包
	packetData[5] = *(((u8*)ADC_Value)+5);
    packetData[6] = *(((u8*)ADC_Value)+2);		  //ADC2 --> Roll
	packetData[7] = *(((u8*)ADC_Value)+3);
	packetData[8] = *(((u8*)ADC_Value)+0);        //ADC1 --> Pitch
	packetData[9] = *(((u8*)ADC_Value)+1);
