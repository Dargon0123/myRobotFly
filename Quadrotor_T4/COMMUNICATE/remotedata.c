...
{
	SI24R1_Controlflag = 1; // --遥控对飞机的连接，表示连接成功，一直表示连接成功，没有成为0的可能性
...
//		}
		...
		{
			SENSER_FLAG_RESET(FLY_MODE); //有头模式（默认）
		}
	}
	//遥控器K2按键
...
void SI24R1_SingalCheck(void)
{
	...
	if(SI24R1_Controlflag) //--飞机成功接收数据 标志位 一直都是为 1 的状态
	{
		if(Airplane_Enable && DataID == PreDataID)//飞机与遥控断开连接 --表示DataID 没有更新 则对应没有收到数据包
		{
