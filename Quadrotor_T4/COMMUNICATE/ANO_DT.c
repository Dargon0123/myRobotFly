...
void ANO_DT_Send_Status(float angle_rol, float angle_pit, float angle_yaw, s32 alt, uint8_t FLY_ENABLEl, uint8_t armed)
{
	...
	data_to_send[_cnt++]=0xAA; //数据帧头 0xAA AA   
	data_to_send[_cnt++]=0xAA;
	data_to_send[_cnt++]=0x01; //数据功能字 01
	data_to_send[_cnt++]=0;    //数据长度 LEN
	
	_temp = (int)(angle_rol*100);     //int16 Roll
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	_temp = (int)(angle_pit*100);     //int16 Pit
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	_temp = (int)(angle_yaw*100);     //int16 Yaw
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	
	data_to_send[_cnt++]=BYTE3(_temp2); //int32 Alt
	data_to_send[_cnt++]=BYTE2(_temp2);
	data_to_send[_cnt++]=BYTE1(_temp2);
	data_to_send[_cnt++]=BYTE0(_temp2);
	
	data_to_send[_cnt++] = FLY_ENABLEl; //飞行模式
	
	data_to_send[_cnt++] = armed;  //Armed 加锁 解锁
