...
void Motor_Test(RC_TYPE* rc_data, uint8_t armed )
{
    int16_t throttle;
    throttle =rc_data->THROTTLE -500;
    if(throttle >0 && armed)
    {
        Moto_Pwm(throttle,throttle,throttle,throttle); //将此数值分配到定时器，输出对应占空比的PWM波
    }
    else
    {
        Moto_Pwm(0,0,0,0); //将此数值分配到定时器，输出对应占空比的PWM波
    }
