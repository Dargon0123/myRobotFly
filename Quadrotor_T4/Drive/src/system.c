...
//	FBM320_Init(); //FBM320初始化(气压计蓝) 
	MOTOR_Init(); //电机输出初始化
	BATT_Init(); //电池电压检测初始化
	PID_ReadFlash(); //Flash中的数据读取 PID参数在这里面调用
...
{
	if(ANO_Scan) //500Hz 2ms一次
    {
        ANO_Scan = 0;
        ANO_DT_Data_Exchange(); //更新数据到上位机
    }
    if(IMU_Scan) //100Hz 10ms一次
    {
        IMU_Scan  = 0;
...
//			Control(&Att_Angle,&Gyr_rad,&RC_Control,Airplane_Enable); //姿态控制
        Motor_Test(&RC_Control, Airplane_Enable ); //电机测试
    }
    if(LED_Scan) //10Hz 100ms一次
    {
        LED_Scan = 0;
        LED_Run();
    ...
    {
        IRQ_Scan = 0;
        SI24R1_SingalCheck(); //2.4G通信检测 指示灯正常与否 可飞行指示流水灯
